...
const AboutCompCom = () => {
  ...
  useEffect(() => {
      ...
      }, [profile.website, profile.size, profile.overview]);

  const handleSave = () => {
    let updatedProfile = { ...profile, website, size, overview };
    updateCompany(updatedProfile)
      .then(() => {
  ...
  return (
    <div className="flex mx-5 justify-between gap-1 flex-col">
      ...
      {!edit && (
        <>
          <div className="text-3xl md-mx:text-2xl sm-mx:text-xl font-semibold flex text-mine-shaft-200 justify-between">
            <span>About</span>{" "}
            <div className="text-bright-sun-400" onClick={() => setEdit(!edit)}>
              <Edit2 size={matches ? "32" : "24"} />
            </div>
          </div>
          <div className="text-lg md-mx:text-lg sm-mx:text-sm text-mine-shaft-200 font-md flex gap-2">
            Website Link :
            <Link
          ...
          <div className="text-lg md-mx:text-lg sm-mx:text-sm text-mine-shaft-200 font-sm flex gap-2">
            Company size : {size}
          </div>
          <div className="text-xl md-mx:text-lg sm-mx:text-sm  text-mine-shaft-300">
            Overview : {overview}
          </div>
        </>
      )}
